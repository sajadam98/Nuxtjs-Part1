      ...
      <b-card no-body id="container">
        ...
        >
          ...
          <div v-if="this.status==='accepted'">
            ...
            <b-spinner variant="success" label="Spinning" :hidden="`${spinner}`"></b-spinner>
          </div>
        </b-card-body>
      </b-card>
    </b-container>
...
export default {
  ...
  fetch({ store, params }) {
    return axios
      .get("https://sajadamiri-demo.herokuapp.com/api/users")
      .then(res => {
        store.dispatch("initialUsers", res.data);
      });
  },
  methods: {
    ...mapActions(["addUser"]),
